...
from lib.ref_video.lightrfb import LightRFB
from lib.ref_video.Res2Net_v1b import res2net50_v1b_26w_4s
...
class Fastnet(nn.Module):
    ...
    def forward(self, x):
        if len(x.shape)==4:  # Single Image
            ...
            x = self.feature_extractor.conv1(x_in)
            x = self.feature_extractor.bn1(x)
            x = self.feature_extractor.relu(x)
            x = self.feature_extractor.maxpool(x)
            x1 = self.feature_extractor.layer1(x)
